...
We maintain automated backup systems to ensure service continuity and data integrity. Our backup infrastructure operates as follows:

- Daily backups are performed automatically
- Backups older than 7 days and up to 30 days are retained as weekly snapshots (only the first backup of each week)
- Backups older than 30 days and up to 180 days are retained as monthly snapshots (only the first backup of each month)
- All backups older than 180 days are automatically and permanently deleted

Our backup system uses full database snapshots (mongodump), which cannot be edited to remove specific user or server data.
While we implement reasonable backup measures, we do not guarantee complete data recovery in all circumstances.
Our backup system is designed for disaster recovery and service continuity, not as a user-facing data recovery service.
