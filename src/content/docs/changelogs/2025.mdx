...
- Sending messages from the dashboard now is ratelimited. It allows up to 20 messages per minute per user per server, with a burst of 2 messages every 5 seconds.

  For example, if you try to send 3 messages in under 5 seconds, the third message will be rate limited and you will have to wait before sending it.
  If you try to send more than 20 messages in a minute, you will also be rate limited.  
  However, if you try to send 2 messages in under 5 seconds, and a third message after some delay, all messages will go through without any issues.
...
- `/whoclaimed` - View who is currently handling the ticket.
